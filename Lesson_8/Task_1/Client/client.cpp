...
{
    if(ui->le_login->text().trimmed().isEmpty()) {
        ui->statusbar->showMessage("Please enter a nickname before joining!");
...
void Client::gotMessage()
{
    ...
    if(message.startsWith("Connected users: ")) {
        message.remove("Connected users: ");

        QList<QString> userList = message.split('\n');
        ...
        ui->pte_chat->setPlainText(ui->pte_chat->toPlainText() + message + "\n");
    }
}
